...
public class AbisActivity  extends AppCompatActivity {

    private ArrayList<CustomerModel> getCustList;
    private ArrayList<MaterialModel> getMatList;
    public ArrayAdapter<String> adapter;
    private List<MaterialModel> getBoItem;
    private DataLogic databaseAccess;
   // private ArrayList<BoModel> BoList = new ArrayList<BoModel>();
   //private AbisAdapter abisAdapter;

  //  private ListView BoView;

    private EditText custTxt,rtvTxt,rtvDteTxt;
    private Dialog listDialog,informationDialog;
    private LinearLayout parentInvView,invntView;
    private RelativeLayout sbmtBtn;

    private String Custcd = "",Rtvno = "",Rtvdate = "",RawItm="",Matuom,Extmat;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_abis);

        databaseAccess = DataLogic.getInstance(this);

        custTxt = (EditText) findViewById(R.id.customerField);
        rtvTxt = (EditText) findViewById(R.id.RtvField);
        rtvDteTxt = (EditText) findViewById(R.id.RtvDteField);
        addNewItm = (FloatingActionButton) findViewById(R.id.AddItemBtn);
        parentInvView = (LinearLayout) findViewById(R.id.layout_abisdets);
        invntView  = (LinearLayout) findViewById(R.id.layout_abisdets);
        sbmtBtn = (RelativeLayout) findViewById(R.id.sbmtRprtBtn);
     //   BoView = (ListView) findViewById(R.id.boListView) ;
       ...
       // BoView.setEmptyView(findViewById(R.id.imageViewNoData));

        custTxt.setFocusable(false);
        rtvDteTxt.setFocusable(false);

        getBoItem = new ArrayList<MaterialModel>();
       // Controls();
        ...
        sbmtBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                List<CustomerModel> getRaw = databaseAccess.showRaw();
                BoModel boData = new BoModel();
                boData.setCustcode(Custcd);
                boData.setBioId(getRaw.get(0).getBioId());
                List<BoModel> getBoItm = databaseAccess.getBoItmToSend(boData);
                if (getBoItm.size()== 0)
                {
                    AwesomeInfoDialog awesomeInfoDialog = new AwesomeInfoDialog(AbisActivity.this);
                    awesomeInfoDialog.setTitle("ABIS");
                ...
                else
                {
                    ...
                    awesomeInfoDialog.setPositiveButtonClick(new Closure() {
                        @Override
                        public void exec() {
                            for (int tosnd = 0; tosnd<=getBoItm.size()-1;tosnd++)
                            {
                                cntr +=1;

                                BoModel bo_  = new BoModel();
                                bo_.setCustcode(Custcd);
                                bo_.setExtmat(getBoItm.get(tosnd).getExtmat());
                                bo_.setBioId(getRaw.get(0).getBioId());
                                databaseAccess.UpdateBoItmSend(bo_);

                                RawItm += getBoItm.get(tosnd).getExtmat()+"-"+getBoItm.get(tosnd).getQty()+"-"+getBoItm.get(tosnd).getExpdte()+ "|";
                                if (cntr == 3)
                                {
                                    toSend = Custcd+"*"+ RawItm.substring(0,RawItm.length()-1)+"*"+getBoItm.get(tosnd).getRtvnum()+"*"+getBoItm.get(tosnd).getRtvdate()+"*"+getBoItm.get(tosnd).getBioId();
                                    SmsManager sms = SmsManager.getDefault();
                                    sms.sendTextMessage(getRaw.get(0).getMobile(),null,"AVR " + toSend,null,null);
                            ...
                            {
                                toSend = Custcd+"*"+ RawItm.substring(0,RawItm.length()-1)+"*"+rtvTxt.getText().toString()+"*"+rtvDteTxt.getText().toString()+"*"+getBoItm.get(0).getBioId();
                            ...
                            DialogControl diag = new DialogControl(AbisActivity.this,2,R.drawable.ic_success,"ABIS Submitted");
                            diag.create();
                        }
                    });
        ...
        rtvDteTxt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final Calendar c = Calendar.getInstance();

                if (rtvDteTxt.length() == 0)
                {
                ...
                {
                    String str =  rtvDteTxt.getText().toString();//InvList.get(position).get(Constant.FIFTH_COLUMN).toString();
                ...
                DatePickerDialog datePickerDialog = new DatePickerDialog(informationDialog.getContext(),
                        new DatePickerDialog.OnDateSetListener() {
                            ...
                            public void onDateSet(DatePicker view, int year,
                                ...
                                rtvDteTxt.setText((monthOfYear + 1) + "/" + dayOfMonth  + "/" +year);
                                try {
                                    Date date1 =  new SimpleDateFormat("MM/dd/yyyy").parse(rtvDteTxt.getText().toString());
                                    SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy");
                                    String strDate = formatter.format(date1);
                                    rtvDteTxt.setText(strDate);
        ...
        addNewItm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String rtv = rtvTxt.getText().toString().trim();
                String rtvdate = rtvDteTxt.getText().toString().trim();

                if (TextUtils.isEmpty(cust))
                {
                    custTxt.setError("Please select customer");
                    DialogControl diag = new DialogControl(AbisActivity.this,1,R.drawable.customer,"Please select customer");
                    diag.create();
                }
                else if (TextUtils.isEmpty(rtv))
                {
                    rtvTxt.setError("Please input rtv number");
                    DialogControl diag = new DialogControl(AbisActivity.this,1,R.drawable.invoicenum,"Please input rtv number");
                    diag.create();
                }
                else if (TextUtils.isEmpty(rtvdate))
                {
                    rtvDteTxt.setError("Please input rtv date");
                    DialogControl diag = new DialogControl(AbisActivity.this,1,R.drawable.calendar,"Please input rtv date");
                    diag.create();
                }
                else
                {
                    LayoutInflater inflater = AbisActivity.this.getLayoutInflater();
                    ...
                    itmTxt.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            adapter = new ArrayAdapter< String >
                                    (AbisActivity.this, android.R.layout.simple_list_item_1,
                                            type);
                        ...
                        public void onClick(View v) {
                            ...
                            else
                            {
                                if (getBoItem.size() == 0)
                                {
                                    ...
                                    BoData.setCustcode(Custcd);
                                    BoData.setRtvnum(rtvTxt.getText().toString());
                                    BoData.setRtvdate(rtvDteTxt.getText().toString());
                                    BoData.setExtmat(Extmat);
                                    BoData.setMatnme(itmTxt.getText().toString());
                                    BoData.setExpdte(expDte.getText().toString());
                                    BoData.setQty(Integer.parseInt(qtyTxt.getText().toString()));
                                    BoData.setBioId(getRaw.get(0).getBioId());
                                    databaseAccess.AddAbisItm(BoData);

                                    actLogs.setCustcode(Custcd);
                                    actLogs.setActvty("ABIS");
                                ...
                                else
                                {
                                    ...
                                    matNme.setMatnme(itmTxt.getText().toString());
                                    matNme.setQty(Integer.parseInt(qtyTxt.getText().toString()));
                                    matNme.setExpdte(expDte.getText().toString());
                                    int CntItm =  databaseAccess.CountItm(matNme,3);
                                    ...
                                    {
                                        DialogControl diag = new DialogControl(AbisActivity.this,1,R.drawable.invoicenum,"Item already exist");
                                        diag.create();
                                    }
                                }
                            }
            ...
            public void onClick(View v) {
                ...
                custView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ...
                        BoModel cst = new BoModel();
                        cst.setCustcode(Custcd);

                        List<BoModel> getAbisData = databaseAccess.getBoData(cst);

                        if (getAbisData.size() != 0)
                        {
                            Custcd = getAbisData.get(0).getCustcode();
                            Rtvno = getAbisData.get(0).getRtvnum();
                            Rtvdate = getAbisData.get(0).getRtvdate();

                            rtvTxt.setText(Rtvno);
                            rtvDteTxt.setText(Rtvdate);
    ...
    private void Controls()
    {
        ...
        cst.setCustcode(Custcd);
        getBoItem = databaseAccess.getMatAbis(cst);
        if (getBoItem.size() == 0)
        {
            LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View noDataView = inflater.inflate(R.layout.layout_nodata, null);
        ...
        else
        {
            ...
            for (int itmInv = 0; itmInv<=getBoItem.size()-1;itmInv++)
            {
                LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                ...
                matNameTxt.setText(getBoItem.get(itmInv).getMatnme()+" ("+getBoItem.get(itmInv).getUom() +")");

                MaterialModel matName = new MaterialModel();
                matName.setMatnme(getBoItem.get(itmInv).getMatnme());
                List<MaterialModel> getMatDets = databaseAccess.getMatAbisDets(matName);
                    ...
                    InvItm.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            addBtn.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    BoModel mats = new BoModel();
                                    mats.setMatnme(getMatDets.get(finalMatdets).getMatnme());
                                    mats.setRtvnum(rtvTxt.getText().toString());
                                    mats.setRtvdate(rtvDteTxt.getText().toString());
                                    mats.setQty(qtyCnt);
                                    mats.setCustcode(Custcd);
                                    mats.setExpdte(expDteTxt.getText().toString());
                                    databaseAccess.UpdateAbisItem(mats);
                                    informationDialog.dismiss();

                                    DialogControl diag = new DialogControl(AbisActivity.this,2,R.drawable.ic_success,"Item updated");
                                    diag.create();
                            ...
                            addMreBtn.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ...
                                    mats.setCustcode(Custcd);
                                    mats.setExpdte(expDteTxt.getText().toString());
                                    databaseAccess.DeleteAbisItem(mats);
                                    informationDialog.dismiss();

                                    DialogControl diag = new DialogControl(AbisActivity.this,3,R.drawable.ic_dialog_info,"Item deleted");
                                    diag.create();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == android.view.KeyEvent.KEYCODE_BACK) {
            ...
            Intent intent = new Intent(AbisActivity.this, MainActivity.class);
